...
// a large island floating in the sky. You gently land in a fluffy pile of leaves. It's quite cold, but you
...
// 
// To get information, once a bag has been loaded with cubes, the Elf will reach into the bag, grab a handful
// of random cubes, show them to you, and then put them back in the bag. He'll do this a few times per game.
// 
// You play several games and record the information from each game (your puzzle input). Each game is
// listed with its ID number (like the 11 in Game 11: ...) followed by a semicolon-separated list of subsets
...
namespace AdventOfCode
{
    internal class Program
    {
        ...
        // Split each "session" of the game by semicolon.
        // Method assumes string begins on the form of "Game :" which is dumb,
        // but it splits that out.
        // 
        // The end result is a dictionary of cube color: number of them found.
        private static Dictionary<string, int> InputToDictionary(string input)
        {
            Dictionary<string, int> result = new();
            var sanitized = input[(input.IndexOf(':') + 2)..];

            foreach (var set in sanitized.Split("; "))
            {
                foreach (var item in set.Split(", "))
                {
                    var pair = item.Split(' ');
                    if (result.ContainsKey(pair[1].Trim()))
                    {
                        var numeric = int.Parse(pair[0]);
                        if (numeric > result[pair[1].Trim()])
                        {
                            result[pair[1].Trim()] = numeric;
                        }
                    }
                    else
                    {
                        result.Add(pair[1].Trim(), int.Parse(pair[0]));
                    }
                }                
            }
            return result;
        ...
        private static bool GameIsPossible(string game, string restrictions)
        {
            // Prepare data; strip each line of beginning marker, and then chunk it.
            var restrictionsData = InputToDictionary(": " + restrictions.Trim());
            var gameBoxes = InputToDictionary(game);

            // Shove it into a dictionary if it exists, otherwise record highest.
            foreach (var item in restrictionsData)
            {
                if (!gameBoxes.ContainsKey(item.Key))
                {
                    return false;
                }

                if (gameBoxes[item.Key] > item.Value) return false;
            }

            return true;
        }

        // Multiply each of the values.
        private static int ComputePower(Dictionary<string, int> list)
        {
            var list2 = list.Select(x => x.Value);
            return list2.Aggregate((a, x) => a * x);
        }

        private static void Main()
        {
            var restriction = "12 red, 13 green, 14 blue";
            var input = FetchStringContents("input_2.txt");
            ...
            foreach (var line in input)
            {
                ...
                if (GameIsPossible(line, restriction))
                {
                    sum += gameCount;
                }
            }
            ...
            var sum2 = input.Aggregate<string?, long>(0, (current, line) 
                => current + ComputePower(InputToDictionary(line)));
